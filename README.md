...
*  ```WEIGHT_PATH = "checkpoints/training_new_non_weighted.pth"``` - this should be the path to the model you are evaluating
...
*   ```DATA_DIR = Path("./data")``` - the data path
*   ```PRED_DIR = Path("./predictions/training_new_non_weighted")``` - the predictions generated by the ```evaluate.py``` script
